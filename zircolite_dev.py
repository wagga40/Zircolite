...
class template_engine:
    ...
    def run(self, data):
        for template, template_output in zip(self.template_paths, self.template_outputs):
            self.logger.info(f'[+] Applying template "{template}", outputting to : {template_output}')
    ...
    def execute_rule(self, rule):
        ...
        if filtered_rows:
            results = {
                "title": title,
                "id": rule_id,
                "description": description.replace("\n", "").replace("\r", "") if self.csv_output else description,
                "sigmafile": filename,
                "sigma": sigma_queries,
...
class gui_generator:
    ...
    def run(self, data):
        self.unzip()
        try:
            self.logger.info(f"[+] Generating ZircoGui package to : {self.output_file}.zip")
            exportforzircoguiTmpl = template_engine([self.template_file], [self.tmp_file], self.time_field)
...
def main():
   ...
   -= Standalone Sigma Detection tool for EVTX/Auditd/Sysmon Linux =-
    ...
    if args.csv and len(args.ruleset) > 1: 
        logger.error(f"{Fore.RED}   [-] Since fields in results can change between rulesets, it is not possible to have CSV output when using multiple rulesets{Fore.RESET}"), sys.exit(2)
    ...
    chosen_input = next((key for key, value in args_dict.items() if "_input" in key and value), None)
    ...
    aggregated_rules = sorted(aggregated_rules.values(), key=lambda d: level_order.get(d.get('rule_level', 'informational'), float('inf'))) # Sort by level
    for rule in aggregated_rules:
        ...
        logger.info(f'{Fore.CYAN}    - {rule_title} [{formatted_level}] : {rule_count} events{Fore.RESET}')
    ...
    if args.package and all_full_results != []:
       if Path("templates/exportForZircoGui.tmpl").is_file() and Path("gui/zircogui.zip").is_file():
           packager = gui_generator("gui/zircogui.zip", "templates/exportForZircoGui.tmpl", None, args.timefield)
