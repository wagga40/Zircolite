...
class StreamingEventProcessor:
    ...
    def __init__(
        ...
        *,
        ...
        if self._has_time_filter:
            self._time_after_parsed = self._parse_time_bound(
                proc.time_after, "1970-01-01T00:00:00"
            )
            self._time_before_parsed = self._parse_time_bound(
                proc.time_before, "9999-12-12T23:59:59"
            )
        ...
        if args_config:
            ...
            self.chosen_input = next(
                (key for key, value in args_dict.items() if "_input" in key and value),
                None,
            )
    ...
    def _flatten_event(
        ...
        def process_leaf(path_parts: Tuple[Any, ...], obj: Any) -> None:
            ...
            if alias_key is None and alias_raw is None:
    ...
    def stream_evtx_events(self, evtx_file: str) -> Generator[dict, None, None]:
        ...
        try:
            ...
            if "Invalid EVTX" in err_msg or "ElfFile0" in err_msg:
                if Path(
                    evtx_file
                ).suffix.lower() == ".7z" or evtx_file.lower().endswith(".json.7z"):
                    self.logger.error(
    ...
    def stream_json_events(
        self, json_file: str, json_array: bool = False
    ...
    def stream_csv_events(self, csv_file: str) -> Generator[dict, None, None]:
        ...
        try:
            ...
            with open_maybe_compressed(
                csv_file, "rt", encoding="utf-8", password=self.archive_password
            ) as f:
    ...
    def stream_json_array_chunked(self, json_file: str) -> Generator[dict, None, None]:
        ...
        try:
            ...
            with open_maybe_compressed(
                json_file, "rt", encoding="utf-8", password=self.archive_password
            ) as f:
    ...
    def process_file_streaming(
        ...
        for event in event_stream:
            ...
            if keepflat_file is not None:
                keepflat_file.write(json.dumps(event))
        ...
        for col in columns:
            ...
            if col_lower not in db_columns:
                ...
                try:
                    ...
                    # Column may already exist - refresh cache from actual schema
                    cursor.execute("PRAGMA table_info(logs)")
                    self._db_columns = {row[1].lower() for row in cursor.fetchall()}
                    db_columns = self._db_columns
